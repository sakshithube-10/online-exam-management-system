<?php
$time = $_POST['time']; // Added to capture test time
...
echo "<script>alert(' Test removed successfully!'); </script>";
...
$tag = $_POST['tag'];
$desc = $_POST['desc'];
$q3=mysqli_query($con,"INSERT INTO quiz (title, sahi, wrong, total, tag, intro, email) VALUES ('$name' , '$sahi' , '$wrong','$total','$time' ,'$desc','$tag' ,'$email')");
...
// edit question
if(isset($_SESSION['key'])){
if(@$_GET['q']== 'editqns' && $_SESSION['key']=='prasanth123') {
$eid=@$_GET['eid'];
$qid=@$_GET['qid'];
...
// Update the question
mysqli_query($con,"UPDATE questions SET qns='$qns' WHERE qid='$qid'") or die('Error updating question');
...
if(count($optids)==4){
    mysqli_query($con,"UPDATE options SET option='$oa' WHERE optionid='$optids[0]'") or die('Error updating option A');
    mysqli_query($con,"UPDATE options SET option='$ob' WHERE optionid='$optids[1]'") or die('Error updating option B');
    mysqli_query($con,"UPDATE options SET option='$oc' WHERE optionid='$optids[2]'") or die('Error updating option C');
    mysqli_query($con,"UPDATE options SET option='$od' WHERE optionid='$optids[3]'") or die('Error updating option D');
}

// Update the correct answer
switch($ans){
case 'a': $ansid=$optids[0]; break;
case 'b': $ansid=$optids[1]; break;
case 'c': $ansid=$optids[2]; break;
case 'd': $ansid=$optids[3]; break;
default: $ansid=$optids[0];
}
mysqli_query($con,"UPDATE answer SET ansid='$ansid' WHERE qid='$qid'") or die('Error updating answer');

echo "<script>alert('Edited successfully!'); window.location.href = 'dash.php';</script>";
exit;
}
}
